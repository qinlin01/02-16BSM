 ...
 */
app.controller('agencyCustomerChangeCtrl', function ($rootScope, $scope, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "agencyCustomerChange/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "agencyCustomerChange/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
                    ...
                    if($scope.VO.update){
                        ...
                        $scope.update.push({
                            colName: '是否集团成员单位',
                            colOldVal: response.result.c1Level,
                            colNewVal: '',
                            colNewVals: 'c1Level'
                        });
                        $scope.update.push({
                            colName: '上级管理单位',
                            colOldVal: response.result.upCustomer.name,
                            colNewVal: '',
                            colNewVals: 'upCustomerSw'
                        });
                        $scope.update.push({
                            colName: '中介公司类型',
                            colOldVal: response.result.enumAgencyCompType,
                            colNewVal: '',
                            colNewVals: 'enumAgencyCompType'
                        });
                        $scope.update.push({
                            colName: '所在区域',
        ...
        $scope.onSaveVO = function () {
            ...
            layer.load(2);
            $http.post($rootScope.basePath + "agencyCustomerChange/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.pkCustomer.name', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                if (newVal) {
                    $http.post($scope.basePath + "agencyCustomer/findOne", {pk: $scope.VO.pkCustomer.pk}).success(function (response) {
                        if (response && response.code == "200") {
                            ...
                            $scope.VO.enumAgencyCompType = response.result.enumAgencyCompType;
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "agencyCustomerChange/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '保险中介公司信息变更.csv',
            columnDefs: [
    ...
    $scope.billdef = "AgencyCustomerChange";
