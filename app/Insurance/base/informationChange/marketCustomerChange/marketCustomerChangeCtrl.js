 ...
 */
app.controller('marketCustomerChangeCtrl', function ($rootScope, $scope, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "marketCustomerChange/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "marketCustomerChange/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
                    ...
                    $scope.VO.pkCustomer.signcustomerno = response.result.signcustomerno;
                    ...
                    if ($scope.VO.update) {
                        ...
                        $scope.update.push({
                            colName: '集团名称',
                            colOldVal: response.result.coCustomerTopName && response.result.coCustomerTopName.name,
                            colNewVal: '',
                            colNewVals: 'coCustomerTopName'
                        });
                    }
                    if ($scope.isGrid) {
                        //$scope.VO = $scope.initVO();
                    ...
                    //清理流程图获取不到id
                    if ($scope.VO.id == null || $scope.VO.id == undefined) {
                        $scope.VO.id = pk;
                    }
                } else {
                    if (response) {
        ...
        $scope.onSaveVO = function () {
            ...
            if ($scope.VO.billstatus == 37) {
                $scope.VO.billstatus = 0;
            }
            $http.post($rootScope.basePath + "marketCustomerChange/save", {data: angular.toJson($scope.VO), funCode: $scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
    ...
    $scope.initWatch = function () {
        $scope.$watch('VO.cusChangeType', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                ...
                /*if (newVal && '市场转国网' == $rootScope.returnSelectName(newVal, 'MARCHANGETYPE')) {
        ...
        $scope.$watch('VO.upCustomerSw.name', function (newVal, oldVal) {
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                if ($scope.VO.cusChangeType == 9) {
                    $scope.VO.c1Level = $scope.VO.upCustomerSw.c_1_type;
                } else {
                    $scope.VO.c1Level = null;
                }
        ...
        $scope.$watch('VO.pkCustomer.name', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                if (newVal) {
                    if ($scope.VO.billstatus == 36 && count == 0) {
                        count++;
                    } else {
                        $http.post($scope.basePath + "marketCustomer/findOne", {pk: $scope.VO.pkCustomer.pk}).success(function (response) {
                            if (response && response.code == "200") {
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "marketCustomerChange/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
        ...
        $scope.onSave = function () {
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    $scope.VO.update = $scope.update;
                    $scope.VO.cusChangeTypeName = $rootScope.returnSelectName($scope.VO.cusChangeType, "MARCHANGETYPE");
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '市场客户信息变更.csv',
            columnDefs: [
                {name: 'serial', displayName: '变更流水号', width: 100,},
                {name: 'c0Code', displayName: '市场客户编号', width: 100,},
                {name: 'signcustomerno', displayName: '市场客户代码', width: 100,},
                {name: 'c0Name', displayName: '市场客户名称', width: 100,},
                ...
                {name: 'cusChangeType', displayName: '变更类型', width: 100, cellFilter: 'SELECT_MARCHANGETYPE'},
    ...
    $scope.billdef = "MarketCustomerChange";
