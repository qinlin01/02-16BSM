 ...
 */
app.controller('insureCustomerCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, $location, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryAllForGrid = function (data) {
            layer.load(2);
            return $http.post($rootScope.basePath + "insureCustomer/queryAllForGrid", {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "insureCustomer/queryForGrid", {
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
        ...
        $scope.findOne = function (pk,ifDesensitize) {
            $scope.pk = pk;
            $http.post($scope.basePath + "insureCustomer/findOne", {pk: pk,ifDesensitize:ifDesensitize}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
        ...
        $scope.onSaveVO = function () {
            ...
            if ($scope.childMust) {
                layer.load(2);
                $http.post($rootScope.basePath + "insureCustomer/save", {data: angular.toJson($scope.VO), funCode: $scope.funCode})
                    .success(function (response) {
                        if (response.code == 200) {
                            $scope.isGrid = false;
                            $scope.isBack = false;
    ...
    $scope.initWatch = function () {
            ...
            if ($scope.form) {
                if ($scope.VO.c1Institution != null) {
                    if ($scope.VO.c1Institution != oldVal) {
                        //当value改变时执行的代码
                        $http.post($scope.basePath + "agencyCustomer/checkIfExist", {c1Institution: $scope.VO.c1Institution, cName: $scope.VO.c0Name}).success(function (response) {
                            layer.closeAll('loading');
                            if (response && response.code != "200") {
        ...
        $scope.$watch('VO.enumInsureCompType', function (newVal, oldVal) {
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                $scope.VO['enumInsureCompTypeName'] = $rootScope.SELECT.INSURECOMPTYPE[newVal - 1].name;
            }
        }, true);
        $scope.$watch('VO.enumInsuranceLevel', function (newVal, oldVal) {
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                $scope.VO['enumInsuranceLevelName'] = $rootScope.SELECT.INSURANCELEVEL[newVal - 1].name;
            }
        }, true);
...
*/
        ...
        $scope.onSubmit = function () {
            ...
            $http.post($scope.basePath + "insureCustomer/submit", {data: angular.toJson(rows[0])}).success(function (response) {
                if (response && response.code == 200) {
                    $scope.queryForGrid($scope.QUERY);
                    if (response) {
                        if (response.msg) {
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "insureCustomer/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '保险公司信息.csv',
            columnDefs: [
                ...
                {name: 'upCustomer.name', displayName: '上级公司', width: 100,},
        ...
        $scope.selectTabName = 'linkmanGridOptions';
