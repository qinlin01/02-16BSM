 ...
 */
app.controller('merchantNumberCtrl', function ($rootScope, $scope, $sce, $http, $q, $stateParams, uiGridConstants, ngDialog, ngVerify, ngTableParams) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryHttp = function (msg,data){
            ...
            layer.load(2);
            $http.post($scope.basePath + "merchantNumber/queryForGrid", {
        ...
        $scope.findOne = function (pk, callback) {
            $scope.pk = pk;
            $http.post($scope.basePath + "merchantNumber/findOne", {id: pk}).success(function (response) {
                layer.closeAll('loading');
        ...
        $scope.onSaveVO = function () {
            ...
            $http({
                method: "POST",
                url: $rootScope.basePath + "merchantNumber/save",
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "merchantNumber/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response.code == 200) {
                            $scope.queryForGrid($scope.QUERY);
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                {name: 'merchantNo', displayName: '商户号信息编号'},
                {name: 'thirdOrgType', displayName: '第三方支付机构类型',cellFilter: 'SELECT_THIRDORGTYPE'},
                {name: 'accountNumber', displayName: '账号'},
                {name: 'accountName', displayName: '账号名称', },
                {name: 'bankAccount', displayName: '银行账号', },
                {name: 'openDate', displayName: '开户时间', },
                {name: 'closeDate', displayName: '销户时间', },
                {name: 'usingType', displayName: '使用状态', cellFilter: 'SELECT_USINGTYPE'},
