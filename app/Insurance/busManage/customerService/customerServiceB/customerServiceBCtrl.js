 ...
 */
app.controller('customerServiceBCtrl', function ($rootScope, $scope, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "customerServiceB/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "customerServiceB/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
        ...
        $scope.onSaveVO = function () {
            layer.load(2);
            $http.post($rootScope.basePath + "customerServiceB/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "customerServiceB/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'deptName', displayName: '联系部门', width: 100,},
                {name: 'contact', displayName: '联系人', width: 100,},
                {name: 'commitDate', displayName: '提交日期', width: 100,},
