app.controller('personalInsuranceArchivesCtrl', function ($rootScope, $scope,$sce, $http, $stateParams,$state, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog,$window) {
    $scope.initData = function (data) {
        ...
        $scope.entityVO = 'nc.vo.busi.personalInsuranceArchivesVO';
        //主表数据
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.archivesPath + "personalInsuranceArchives/queryForGrid",{
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
        ...
        $scope.findOne = function (id,callback) {
            $scope.id = id;
            $http.post($scope.archivesPath + "personalInsuranceArchives/findOne",{pk: id}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200"){
                    ...
                    if ($scope.isEdit == true && ($scope.VO.bussinessStatus == 1 || $scope.VO.bussinessStatus == null)) {
                        if (response.result.projectInsurance != null) {
                            $scope.VO.fileStatusList.insurancePolicy = response.result.projectInsurance.length;
                        }
        ...
        $scope.onSaveVO = function () {
            ...
            $http.post($scope.archivesPath + "personalInsuranceArchives/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
        ...
        $scope.onRefresh = function () {
            ...
            layer.confirm('是否刷新选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.archivesPath + "personalInsuranceArchives/refresh", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
        ...
        $scope.onRefreshAll = function () {
            layer.confirm('刷新全量数据有风险，建议做好数据备份。是否进行全量刷新？', {
                ...
                function () {
                    layer.load(2);
                    $http.post($scope.archivesPath + "personalInsuranceArchives/refreshAll").success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('刷新成功!', {
    ...
    $scope.initFunction = function () {
        ...
        $scope.initField = function (){
            ...
            if(!$scope.VO.customerServiceList){
                ...
                $scope.VO.customerServiceList.assistClaim = 0;
            }else{
                ...
                if(!$scope.VO.customerServiceList.assistClaim){
                    $scope.VO.customerServiceList.assistClaim = 0;
                }
            }
            //暂保申请
            ...
            //出单通知书
            if(!$scope.VO.fileStatusList.issueNotices){
                $scope.VO.fileStatusList.issueNotices = 0;
            }
            //暂保单
            if(!$scope.VO.fileStatusList.coverNote){
                $scope.VO.fileStatusList.coverNote = 0;
            }
            //保险方案
            if(!$scope.VO.fileStatusList.insuranceSchemes){
                $scope.VO.fileStatusList.insuranceSchemes = 0;
            }
            //保险方案
            if(!$scope.VO.fileStatusList.insuranceCooperation){
                $scope.VO.fileStatusList.insuranceCooperation = 0;
            }
        }
        ...
        $scope.countFileStatusList = function () {
            ...
            for (var i = 0; i < mainContractGridOptionsArray.length; i++) {
                if (mainContractGridOptionsArray[i].file_type == 1) {
                    $scope.VO.fileStatusList.insuranceCooperation = $scope.VO.fileStatusList.insuranceCooperation + 1;
                }
            }
            ...
            $scope.VO.fileStatusList.powerOfAttorney = 0;
            ...
            for (var i = 0; i < dealAttachmentBGridOptionsArray.length; i++) {
                ...
                if (dealAttachmentBGridOptionsArray[i].file_type == 4) {
                    $scope.VO.fileStatusList.powerOfAttorney = $scope.VO.fileStatusList.powerOfAttorney + 1;
                }
            }
            ...
            for (var i = 0; i < customerServiceGridOptionsArray.length; i++) {
                ...
                if (customerServiceGridOptionsArray[i].file_type == 4) {
                    $scope.VO.customerServiceList.assistClaim = $scope.VO.customerServiceList.assistClaim + 1;
                }
            }
            ...
            for (var i = 0; i < billApplyGridOptionsArray.length; i++) {
                if (billApplyGridOptionsArray[i].file_type == 1) {
                    $scope.VO.fileStatusList.coverNote = $scope.VO.fileStatusList.coverNote + 1;
                }
                if (billApplyGridOptionsArray[i].file_type == 2) {
                    $scope.VO.fileStatusList.issueNotices = $scope.VO.fileStatusList.issueNotices + 1;
                }
                if (billApplyGridOptionsArray[i].file_type == 3) {
                    $scope.VO.fileStatusList.insuranceSchemes = $scope.VO.fileStatusList.insuranceSchemes + 1;
                }
                if (billApplyGridOptionsArray[i].file_type == 4) {
    ...
    $scope.initButton = function () {
        ...
        $scope.onGetTrainingMaterials = function (){
            ...
            ngDialog.openConfirm({
                showClose: true,
                closeByDocument: false,
                template: 'view/personalInsuranceArchives/trainingMaterials.html',
                className: 'ngdialog-theme-formInfo',
                scope: $scope,
                rootScope: $rootScope,
                preCloseCallback: function (value) {
        ...
        $scope.onUploads = function (name) {
            ...
            if ($scope.selectTabName == "baseFilesGridOptions"){
                // $scope.aVO = {"nameArray":[{id:1,name:"客户告知书",title:"（新增客户必须上传）"},{id:2,name:"保险方案确认函",title:"（股东业务必留存）"},{id:3,name:"保险到期提示函",title:"（续保业务必须上传）"}] };
                $scope.aVO = {"nameArray":[{id:1,name:"客户告知书",title:"（新增客户必须上传）"}] };
                ...
                $scope.aVO = {"nameArray": [{id:1,name:"领取报酬情况表"},{id:2,name:"其他档案"},{id:3,name:"装盒照片"},{id:4,name:"客户授权委托书"}]};
            }else if ($scope.selectTabName == "customerServiceGridOptions"){
                $scope.aVO = {"nameArray": [{id: 1, name: "客户服务计划表"}, {id: 2, name: "客户服务工作报告"},{id: 4, name: "协助索赔资料"}]};
            }else if ($scope.selectTabName == "billApplyGridOptions"){
                $scope.aVO = {"nameArray": [{id: 1, name: "暂保单"}, {id: 2, name: "出单通知书"}, {id: 3, name: "保险方案/保险建议书"},{id: 4, name: "暂保申请"},{id: 5, name: "保单批改申请书"},{id: 6, name: "保险单"}]};
            }
        ...
        $scope.onDownLoads = function(id){
            var data =[];
            if ($scope.downCommon($scope.VO.junctionFiles,0)||$scope.downCommon($scope.VO.appendices,1)||$scope.downCommon($scope.VO.dealAttachmentB,2)||$scope.downCommon($scope.VO.projectInsurance,3)||$scope.downCommon($scope.VO.changeBillCheck,4)){

                data.push($scope.downCommon($scope.VO.junctionFiles,0),$scope.downCommon($scope.VO.appendices,1),$scope.downCommon($scope.VO.dealAttachmentB,2),$scope.downCommon($scope.VO.projectInsurance,3),$scope.downCommon($scope.VO.changeBillCheck,4));
            }
        ...
        $scope.downCommon = function(row,type){
            var tableName=["结算档案","合同附件","其他附件","保单附件","批单附件"];
            var fileName=[[{id:1,name:"保险公司结算单"},{id:2,name:"业务收入确认书"},{id:3,name:"发票开具申请单"},{id:4,name:"发票扫描件"}],[{id:1,name:"保险公司合作协议"}],[{id:1,name:"领取报酬情况表"},{id:2,name:"其他档案"},{id:3,name:"装盒照片"},{id:4,name:"客户授权委托书"}]];
            var returnData={};
     ...
     /*   /!**
        ...
        $scope.onSave = function (data) {
            ...
            if (data) {
                ...
                data.fbillstatus=1;
                var tablename = $scope.table_name;
                $http.post($scope.archivesPath  + "personalInsuranceArchives/save",{data:angular.toJson(data),tablename:tablename}).success(function(response) {
                    if(response.code == 200){
                        $scope.isGrid = false;
                        $scope.isBack = false;
                        $scope.isEdit = true;
            ...
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    if ($scope.VO.bussinessStatus == 1){
                        ...
                        if($scope.VO.fileStatusList.insuranceCooperation == 0){
                            ifN =true;
                            return layer.alert("请上传保险公司合作协议!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        ...
                        if($scope.VO.fileStatusList.powerOfAttorney == 0){
                            ifN =true;
                            return layer.alert("请上传客户授权委托书!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        ...
                        // if($scope.VO.fileStatusList.schedule == 0){
                        //     ifN =true;
                        //     return layer.alert("请上传客户服务计划表!",
                        //         {skin: 'layui-layer-lan', closeBtn: 1});
                        // }
                        // if($scope.VO.fileStatusList.workReport == 0){
                        //     ifN =true;
                    ...
                    if ($scope.VO.period !=null){
                        ...
                        if (pkProject != null && pkProject.length>0){
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'projectType', displayName: '险种分类',cellFilter: 'SELECT_MARKETTYPE'},
            ...
            columnDefs: [
                {
                    ...
                    displayName: '档案类型',
                    enableCellEdit: true,
                    cellFilter: 'SELECT_PINSURANCEFILETYPE1',
            ...
            columnDefs: [
                {
                    ...
                    displayName: '合同类型',
                    enableCellEdit: true,
                    cellFilter: 'SELECT_PINSURANCEFILETYPE2',
                    editableCellTemplate: 'ui-grid/dropdownEditor'
                    ,
                    editDropdownValueLabel: 'name'
                    ,
            ...
            columnDefs: [
                {
                    ...
                    displayName: '附件类型',
                    enableCellEdit: true,
                    cellFilter: 'SELECT_PINSURANCEFILETYPE3',
                    editableCellTemplate: 'ui-grid/dropdownEditor'
                    ,
                    editDropdownValueLabel: 'name'
                    ,
            ...
            columnDefs: [
                {
                    ...
                    displayName: '档案类型',
                    enableCellEdit: false,
                    cellFilter: 'SELECT_FILETYPE1FORPA',
                    editableCellTemplate: 'ui-grid/dropdownEditor'
                    ,
                    editDropdownValueLabel: 'name'
                    ,
                    editDropdownOptionsArray: getSelectOptionData.FILETYPE1FORPA
                },
                {
                    name: 'attachment_name', displayName: '档案名称', enableCellEdit: false
                },
    ...
    $scope.table_name = "lr_insurance_archives";
