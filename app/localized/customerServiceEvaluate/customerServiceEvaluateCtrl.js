app.controller('customerServiceEvaluateCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify) {
    $scope.initData =function (data) {
        ...
        $scope.entityVO = 'nc.vo.busi.customerServiceEvaluateVO';
        //主表数据
    ...
    $scope.initHttp=function(){
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post(localizedPath  + "customerServiceEvaluateController/queryForGrid", {
    ...
    $scope.initButton = function(){
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post(localizedPath + "customerServiceEvaluateController/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
        ...
        $scope.findOne = function (pk,callback) {
            $scope.pk = pk;
            $http.post(localizedPath  + "customerServiceEvaluateController/findOne",{pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200"){
                ...
                if (value != null) {
                    ...
                    if ($scope.dealAttachmentBGridOptions.data) {
                        angular.forEach(value, function (item) {
                            let dealAttachmentB ={};
                            dealAttachmentB.file_type="客户服务工作报告管理";
        ...
        $scope.onCard = function (id) {
            ...
            $http.post( localizedPath  + "customerServiceEvaluateController/findOne",{
                pk: id,
            ...
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    if (data) {
                        ...
                        $http.post( localizedPath  + "customerServiceEvaluateController/save",{data:angular.toJson(data),tablename:tablename}).success(function(response) {
                            if(response.code == 200){
                                $scope.isGrid = false;
                                $scope.isBack = false;
                                $scope.isEdit = true;
        ...
        $scope.onTemporary = function (data) {
            ...
            if (data) {
                data=$scope.VO;
                var tablename = $scope.table_name;
                $http.post( localizedPath  + "customerServiceEvaluateController/onTemporary",{data:angular.toJson(data),tablename:tablename}).success(function(response) {
                    if(response.code == 200){
                        $scope.isGrid = false;
                        $scope.isBack = false;
                        $scope.isEdit = true;
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'evaluateYear', displayName: '报告年度'},
    ...
    $scope.table_name = " lr_customerservice_evaluate";
    $scope.billdef = "CustomerServiceEvaluate";
    $scope.initData();
    $scope.initHttp();
    $scope.initFunction();
    $scope.initButton();
