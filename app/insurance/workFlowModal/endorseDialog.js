/**
 * Created by jiaoshy on 2017/7/3.
 */
(function (root, factory) {
    ...
    var m = angular.module('endorseDialog', []);

    m.provider('endorseDialog', function () {
            ...
            function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller, $injector, ngDialog) {
                ...
                var publicMethods = {
                    ...
                    open: function (opts) {
                        ...
                        // alert(msg)
                        $http.post($rootScope.basePath + "workFlowMadelController/queryEndorseInfo", {data: angular.toJson(data)}).success(function (response) {
                            if (response && response.code == "200") {
                                ...
                                ngDialog.open({
                                    template: '../insurance/view/workFlow/endorseDialog.html',
                                    controller: 'endorseController',
                                    data: {
                                        config: _config,
                                        param: param
                                    },
    ...
    m.controller('endorseController', ['$scope', '$rootScope', '$http', 'ngDialog', 'endorseDialog', '$sce', '$location', 'unPassDialog', function ($scope, $rootScope, $http, ngDialog, endorseDialog, $sce, $location,  unPassDialog) {
        ...
        $scope.initFunction = function () {
            ...
            $scope.onSubmitConfirm = function () {
                ...
                if ($scope.RIGHT_DATA == null || $scope.RIGHT_DATA.length != 1) {
                    return angular.alert('请选择一个用户!', {skin: 'layui-layer-lan', closeBtn: 1});
                }
                ...
                $http.post($rootScope.basePath + "workFlowMadelController/EndorseSubmitConfirm", {data: angular.toJson($scope.submitVO)}).success(function (response) {
                    if (response && response.code == "200") {
                        angular.alert($rootScope.getDisName(response.msg, '', ''));
                        ngDialog.close();
