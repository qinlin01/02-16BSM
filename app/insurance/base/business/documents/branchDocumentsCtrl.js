 ...
 */
app.controller('branchDocumentsCtrl', function ($rootScope, $scope, $http, $stateParams, uiGridConstants, ngDialog, ngVerify,$location, activitiModal,workFlowDialog) {
    ...
    $scope.initHttp = function () {
        $scope.queryForGridTree = function () {
            layer.load(2);
            $http.post($scope.basePath + "branchDocuments/queryForGridTree").success(function (response) {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "branchDocuments/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "branchDocuments/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
        ...
        $scope.onSaveVO = function () {
            layer.load(2);
            $http.post($rootScope.basePath + "branchDocuments/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
            ...
            if ($scope.dealAttachmentBGridOptions.data.length != 0) {
                ...
                ngDialog.openConfirm({
                    ...
                    if (value != null) {
                        ...
                        $scope.VO.addFile = $scope.dealAttachmentBGridOptions.data[0].pk_pub_blob.pk_pub_blob;
...
*/
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "branchDocuments/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '分公司模板管理.csv',
            columnDefs: [
