 ...
 */
app.controller('againBusinessExamineCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog) {
            ...
            if ($scope.isEdit) {
                ...
                if (newVal &&null!=$scope.VO.pkProject&&$scope.VO.pkProject.code) {
                    ...
                    $scope.VO.docTitle = '关于' + year + '年' + $scope.VO.pkProject.cinsureman_name
                        + '  ' + $rootScope.returnSelectName(newVal, 'AGAINBUSINESSEXAMINEDOCTYPE')
                        + '签报';
        ...
        $scope.$watch('VO.pkProject.code', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                if (newVal && $scope.VO.enumDocType) {
                    var year = new Date().format("yyyy");
                    $scope.VO.docTitle = '关于' + year + '年' + $scope.VO.pkProject.cinsureman_name
                        + '  ' + $rootScope.returnSelectName($scope.VO.enumDocType, 'AGAINBUSINESSEXAMINEDOCTYPE')
                        + '签报';
                    var year = $scope.VO.docTitle.substr(2,4);
                    if(year!=$scope.VO.pkProject.name.substr(0,4)){
                        angular.alert("请确认文件标题中年度是否正确！");
    ...
    $scope.initButton = function () {
        ...
        $scope.onViewAttachment = function () {
            ...
            ngDialog.openConfirm({
                ...
                template: 'view/againBusinessExamine/attachment.html',
        ...
        $scope.onInsurance = function () {
            ngDialog.openConfirm({
                showClose: true,
                closeByDocument: false,
                template: 'view/againBusinessExamine/InsuranceBill.html',
                className: 'ngdialog-theme-formInfo',
                scope: $scope,
                preCloseCallback: function (value) {
                    if (value && value == "clear") {
        ...
        $scope.onSave = function () {
            ...
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    $scope.VO.enumDocTypeName = $rootScope.returnSelectName($scope.VO.enumDocType, "AGAINBUSINESSEXAMINEDOCTYPE");
    ...
    $scope.initPage = function () {
        ...
        $scope.card = false;
        $scope.isClear = false;
        $scope.htmlPathCheckBill = 'view/againBusinessExamine/printCheckBill.html';
        $scope.htmlPathReportBill = 'view/againBusinessExamine/printReportBill.html';
        $scope.isGrid = true;
        $scope.isCard = false;
        $scope.isEdit = false;
        $scope.isDisabled = true;
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'enumDocType', displayName: '文件类型', width: 100, cellFilter: 'SELECT_AGAINBUSINESSEXAMINEDOCTYPE'},
