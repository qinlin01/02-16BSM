 ...
 */
app.controller('rechargeManageAuditCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog,$location) {
    ...
    $scope.initButton = function () {
        ...
        $scope.sendvoucher = function () {
            ...
            $http.post($rootScope.basePath + "rechargeManage/sendVoucher", {data: angular.toJson($scope.VO)})
                .success(function (response) {
                    ...
                    if (response.code == 200) {
                        ...
                        ngDialog.openConfirm({
                            ...
                            template: 'view/rechargeManageAudit/voucher.html',
        ...
        $scope.viewvoucher = function () {
            ...
            $http.post($rootScope.basePath + "rechargeManage/viewvoucher", {data: angular.toJson($scope.VO)})
                .success(function (response) {
                    if (response.code == 456) {
                        //layer.msg(response.msg);
                        layer.alert(response.msg, {skin: 'layui-layer-lan', closeBtn: 1});
                    ...
                    if (response.code == 200) {
                        ...
                        ngDialog.openConfirm({
                            showClose: true,
                            closeByDocument: false,
                            template: 'view/rechargeManageAudit/voucherView.html',
                            className: 'ngdialog-theme-formInfo',
                            scope: $scope,
                            preCloseCallback: function (value) {
                                if (value && value == "clear") {
        ...
        $scope.onVoucherUnPass = function () {
            layer.confirm('确认当前结算数据不准确吗？窗口将关闭', {
                ...
                function () {
                    ...
                    $http.post($scope.basePath + "rechargeManage/unpassVoucher", {data: angular.toJson($scope.VO)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            layer.msg('完成!', {
                                shift: 6,
        ...
        $scope.onVoucherPass = function () {
            layer.confirm('确认发送当前的凭证到SAP吗？', {
                ...
                function (index) {
                    layer.close(index);
                    layer.load(2);
                    $http.post($scope.basePath + "rechargeManage/sendVoucherToSap", {data: angular.toJson($scope.VO)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == 200) {
                            //关闭SAP凭证的窗口
                            ngDialog.close();
         ...
         * 审核
         */
        $scope.onRechargeManageAudit = function (){
            ...
            $scope.findOne(rows[0].id,function (){
                ...
                if($scope.VO.factCustomer==null){
                    var data={name:$scope.VO.accountStatement[0].reciprocal_name.name};
    ...
    $scope.beanName ="insurance.rechargeRecordService";
    $scope.initData();
    $scope.initHttp();
    $scope.initFunction();
    $scope.initWatch();
