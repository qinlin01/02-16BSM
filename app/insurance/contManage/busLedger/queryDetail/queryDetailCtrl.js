...
app.controller('queryDetailCtrl', function ($rootScope, $scope, $http) {
    ...
    $scope.initQUERY = function () {
        return {
            year: new Date().getFullYear(), if_forword: 'N'
        }
    };
    $scope.funCode = '30802';
    ...
    $scope.queryForGrid = function (data) {
        ...
        $http.post($scope.basePath + "queryDetail/queryForReport", {
    ...
    $scope.queryAllForGrid = function (data, fun) {
        ...
        $http.post($scope.basePath + "queryDetail/queryAllForGrid", {
            params: angular.toJson(data),
            page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
    ...
    $scope.onConfirmData = function () {
        ...
        $http.post("http://20.176.5.202:18888/mdetail?key=8802538618d5dbd64dc208efe853866e&param=" + (year + month)).success(function (response) {
            layer.closeAll('loading');
            if (response && response.code == "200") {
                layer.alert("本月数据已经发送综合信息管理系统", {skin: 'layui-layer-lan', closeBtn: 1});
            } else {
                if (response) {
                    if (response.msg) {
                        // e.g. 字符转换为Entity Name
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'cm_insurancemoney', displayName: '签单保费(元)', width: 100,},
                {name: 'receivemount', displayName: '应收保费(元)', width: 100,},
                {name: 'receivemounted', displayName: '已收保费(元)', width: 100,},
                // {name: 'unreceivemount', displayName: '未收保费(元)', width: 100,},
                ...
                // {name: 'unreceivefeemount', displayName: '未开票收入(元)', width: 100,},
                ...
                // {name: 'unpaymoun', displayName: '未付保费(元)', width: 100,},
                ...
                {name: 'cost_name', displayName: '成本中心', width: 100,},
                ...
                {name: 'account_date', displayName: '实际到款日期', width: 100,},
