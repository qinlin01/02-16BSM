...
app.controller('insuranceDataCtrl', function ($rootScope, $scope,$sce, $http,$state, $stateParams, uiGridConstants, ngDialog) {
    ...
    $scope.initHttp = function () {
        $scope.queryAllForGridd = function (data) {
            layer.load(2);
            return $http.post($rootScope.basePath + "insuranceData/queryAllForGridd", {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "insuranceData/queryForGrid", {
                params: angular.toJson(data),
        ...
        $scope.moveDianShangData=function () {
            $http.post($scope.basePath + "insuranceData/moveDianShangData").success(function (response) {
            });
        }
        $scope.findOne = function (pk,callback) {
            $scope.pk = pk;
            $http.post($scope.basePath + "insuranceData/findOne", {id: pk}).success(function (response) {
                layer.closeAll('loading');
    ...
    $scope.queryAllForGridd = function (data) {
        layer.load(2);
        return $http.post($rootScope.basePath + "insuranceData/queryForGrid", {
            params: angular.toJson(data),
            ifExport: true
        })
            .success(function (response) {
        ...
        $scope.onPerfect = function (){
            ...
            if("财产险" == flag){
                ...
                if(esName.indexOf("*") >-1){
                    ...
                    // fun_code ="3020501";
                return layer.alert("车险暂时不支持完善保单功能!", {skin: 'layui-layer-lan', closeBtn: 1});
            }
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                {name: 'COMPANY_INFO', displayName: '保险公司名称', width: 200,},
                {name: 'INFO_NUM', displayName: '保单号', width: 200,  },
                {name: 'FLAG', displayName: '保单类型', width: 100,  },
                {name: 'NAME', displayName: '投保人名称', width: 200,  },
                {name: 'CARD_NO', displayName: '投保人证件号', width: 100,  },
                {name: 'POSTAL_ADDRESS', displayName: '投保人地址', width: 150,},
                {name: 'INSURE_START_DATE', displayName: '保单开始日期', width: 100,cellFilter: 'date:"yyyy-MM-dd"',  editableCellTemplate: 'ui-grid/refDate'},
                {name: 'INSURE_END_DATE', displayName: '保单到期日期', width: 100,cellFilter: 'date:"yyyy-MM-dd"',  editableCellTemplate: 'ui-grid/refDate'},
                {name: 'ALL_FEE', displayName: '保险金额/赔偿限额/(元)', width: 100, cellFilter: 'AMOUNT_FILTER',  cellClass: function () { return "lr-text-right"}},
                {name: 'INSURE_FEE', displayName: '保费(元)', width: 100,cellFilter: 'AMOUNT_FILTER',  cellClass: function () { return "lr-text-right"}},
                {name: 'BROKER_NAME', displayName: '经纪公司名称', width: 150,},
                {name: 'POST_TIME', displayName: '推送时间', width: 150, },

            ],
            data: [],
            exporterAllDataFn: function () {
                return $scope.queryAllForGridd($scope.QUERY).then(function () {
