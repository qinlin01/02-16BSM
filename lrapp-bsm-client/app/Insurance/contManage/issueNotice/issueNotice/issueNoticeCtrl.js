 ...
 * 出单通知书
 **/
app.controller('issueNoticeCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, $location, activitiModal) {
    ...
    $scope.initData = function (data) {
        ...
        $scope.addlistOptions = function () {
            $scope.nowNumber++;
            $scope.isShowBEHOLDER = $scope.VO.insureType[$scope.nowNumber].INSURE_TYPE_ID == 'constructionPay' ? true : false;
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "issueNotice/queryForGrid", {
        ...
        $scope.onCreateInsurance = function () {

            ngDialog.openConfirm({
                showClose: true,
                closeByDocument: false,
                template: 'view/issueNotice/createInsurance.html',
                className: 'ngdialog-theme-formInfo',
                controller: 'createInsuranceCtrl',
                scope: $scope,
                rootScope: $rootScope,
            }).then(function (value) {
        ...
        $scope.onSaveVO = function () {
            ...
            $http.post($rootScope.basePath + "issueNotice/save",
                {data: angular.toJson($scope.VO), funCode: $scope.funCode})
    ...
    $scope.initFunction = function () {
        $scope.countALL_FEE = function () {
            for (var i = 0; i < $scope.VO.insureType.length; i++) {
                ...
                if ($scope.VO.insureType[i].INSURE_TYPE_ID == "tuantywx") {
                    for (var it = 0; it < $scope.listOptions[i].insuranceGLifeOptions.data.length; it++) {
                        ...
                        if (data.insuranceMoneyAll != null) {
                            let insuranceMoneyAll = data.insuranceMoneyAll.toString();
                            insuranceMoneyAll = insuranceMoneyAll.replace(/,/g, "");
                            $scope.VO.insureType[i].ALL_FEE = (parseFloat($scope.VO.insureType[i].ALL_FEE) + (parseFloat(insuranceMoneyAll) * 10000)).toFixed(2) * 1;

                        }
                    }
                }

                if ($scope.VO.insureType[i].INSURE_TYPE_ID == "chongdzzhbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "anqsczrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongzzrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "diaozzrbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongydszzrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongdzrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "canycszrbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "chuanbwrzrbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "susbqzrbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongysyqyjyzrbx") {
                ...
                if ($scope.VO.insureType[i].INSURE_TYPE_ID == "constructionPay") {
                    if ($scope.listOptions[i].insurebeHoldeGridOptions != null) {
                        for (let j = 0; j < $scope.listOptions[i].insurebeHoldeGridOptions.data.length; j++) {
                            if (!$scope.listOptions[i].insurebeHoldeGridOptions.data[j].BEHOLDER_INSURANCEMONEY) {
                                $scope.listOptions[i].insurebeHoldeGridOptions.data[j].BEHOLDER_INSURANCEMONEY = 0;
                            }
                            $scope.VO.insureType[i].ALL_FEE = (parseFloat($scope.VO.insureType[i].ALL_FEE) + parseFloat($scope.listOptions[i].insurebeHoldeGridOptions.data[j].BEHOLDER_INSURANCEMONEY)).toFixed(2) * 1;
                        }
                    }
                }
            ...
            $http.post($scope.basePath + "insuranceScheme/findOne", {pk: pk}).success(function (response) {
                ...
                if (response && response.code == "200") {
                    ...
                    if(!"34"==dezcBillstatus){
                        return layer.alert('该保险方案未在电网资产系统中审核通过，暂不能使用', {skin: 'layui-layer-lan', closeBtn: 1});
                    }
                    if (null != response.result.name) {
                        var insuranceSchemeName = angular.copy(response.result.name);
                    }
                    ...
                    $scope.VO.schemeName = insuranceSchemeName;
                    // $scope.dealAttachmentBGridOptions.data = $scope.VO.dealAttachmentB;
                    $scope.listOptions = [];
                    $scope.nowNumber = -1;
                    for (var j = 0; j < response.result.insureType.length; j++) {
                        ...
                        $scope.VO.insureType[j].INSURE_PEOPLE_NUM = response.result.insureType[j].INSURE_PEOPLE_NUM;
                        // $scope.listOptions[j].insuranceItemsOptions.data = response.result.insureType[j].insurebeItems;
                        for (var iits = 0; iits < response.result.insureType[j].insurebeItems.length; iits++) {
                            if (response.result.insureType[j].insurebeItems[iits].assetscode != 'gdzc')
                                $scope.listOptions[j].insuranceItemsOptions.data.push(response.result.insureType[j].insurebeItems[iits]);
                        }
        ...
        $scope.onImportUploadFile = function (index) {
            // $("#inputFile"+index).click();
            var id = "#inputFile" + index;
            $(id).click();
        }
        //上传资产数据
        $scope.onImportUploads = function (index) {
            ...
            if (null == $scope.VO.insureType[indexs].downLoadDate || $scope.VO.insureType[indexs].downLoadDate == "") {
                var obj = document.getElementById('inputFile' + index);
                obj.outerHTML = obj.outerHTML;
                return layer.alert('上传时系统将查询资产，请选择资产日期!', {skin: 'layui-layer-lan', closeBtn: 1});
            }
            var file = document.getElementById("inputFile" + index).files[0];
            if (null == file) {
                return;
            }
            if (null == $scope.VO.insureType[indexs].downLoadDate || $scope.VO.insureType[indexs].downLoadDate == "") {
            ...
            if (file != null) {
                ...
                $http({
                    method: 'POST',
                    url: $scope.basePath + "issueNotice/uploadAssets",
                    data: form,
                    ...
                    for (var h = 0; h < $scope.listOptions[indexs].insurebeHoldeGridOptions.data.length; h++) {
                        for (var d = 0; d < detaiil.length; d++) {
                            if ($scope.listOptions[indexs].insurebeHoldeGridOptions.data[h].BEHOLDER_NAME == detaiil[d].assetsName) {
        ...
        $scope.getProjectdetail = function (pk) {
            $http.post($scope.basePath + "issueNotice/getProjectdetail", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
        ...
        $scope.changeInsurebeHolde = function (data, indexs) {
            $http.post($scope.basePath + "issueNoticeGlife/changeInsurebeHolde", {
                ...
                if (response) {
                    ...
                    var detaiil = response.derid;
                    $scope.treedata = response.newDerid;
                    if (null != response.oldAssetsList) {
                        $scope.VO.insureType[indexs].oldAssetsList = response.oldAssetsList;
                    }
                    $scope.VO.insureType[indexs].detaiil = response;
                }
            });
        };
    };
    ...
    $scope.getAssetsData = function (fun, isPrint, i, type) {
        ...
        if (null == $scope.listOptions[i].insurebeHoldeGridOptions.data || $scope.listOptions[i].insurebeHoldeGridOptions.data.length == 0) {
            return layer.alert('请选择被保险人!', {skin: 'layui-layer-lan', closeBtn: 1});
        }
    ...
    $scope.findHistoryData = function (i) {
        ...
        $http.post($scope.basePath + "insuranceScheme/findHistoryData",
            {data: angular.toJson({holderId: $scope.VO.estimatepk.pk, insuretypeId: $scope.VO.insureType[i].INSURE_TYPE_ID})}).success(function (response) {
            layer.closeAll('loading');
            if (response && response.code == "200") {
    ...
    //
    //         }
    //         $http.post($rootScope.basePath + "issueNotice/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
    //             .success(function (response) {
    //
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.insuranceSchemeName', function (newVal, oldVal) {
            if (!$scope.isEdit) {
                return;
            }
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                if ($scope.VO.insuranceSchemeName != $scope.VO.schemeName) {
                    $scope.VO.insuranceSchemeName = $scope.VO.schemeName;
                }
            }
        }, true);
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "issueNotice/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
                ...
                if (errEls && errEls.length) {
                    ...
                    for (var i = 0; i < $scope.VO.insureType.length; i++) {
                        ...
                        if (null == $scope.listOptions[i].insureCompanyGridOptions.data) {
                            return layer.alert(insureTypeName + "请选择承保公司!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        ...
                        if (count != 100) {
                            return layer.alert(insureTypeName + "承保比例必须合计等于100%!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID != "tuantywx" && (null == $scope.VO.insureType[i].FREE_FEE || "<p><br></p>" == $scope.VO.insureType[i].FREE_FEE)) {
                            return layer.alert(insureTypeName + "免赔额信息不能为空!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID != "tuantywx" && (null == $scope.VO.insureType[i].PAY_WAY_CONTRACT || "<p><br></p>" == $scope.VO.insureType[i].PAY_WAY_CONTRACT)) {
                        ...
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID == "jisyqxjzrx") {
                            ...
                            for (let j = 0; j < $scope.listOptions[i].insureMoneyGridOptions.data.length; j++) {
                                var insureObject = $scope.listOptions[i].insureMoneyGridOptions.data[j];
                                var columnDefs = $scope.listOptions[i].insureMoneyGridOptions.columnDefs
                                for (let k = 0; k < columnDefs.length; k++) {
                                    var column = insureObject[columnDefs[k].name]
                                    if (column == null || "" == column) {
                                        return layer.alert(insureTypeName + "【机身一切险及责任险】保险标的(" + columnDefs[k].displayName + ')不能为空',
                                            {skin: 'layui-layer-lan', closeBtn: 1});
                                    }
                                }
                            }
                        ...
                        if (null == $scope.VO.insureType[i].specialPromise || "<p><br></p>" == $scope.VO.insureType[i].specialPromise) {
                            return layer.alert(insureTypeName + "特别约定内容信息不能为空!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if (null == $scope.VO.insureType[i].justiceManage || "<p><br></p>" == $scope.VO.insureType[i].justiceManage) {
                            return layer.alert(insureTypeName + "司法管辖信息不能为空!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if (null == $scope.VO.insureType[i].insurebeExt || "<p><br></p>" == $scope.VO.insureType[i].insurebeExt) {
                            return layer.alert(insureTypeName + "扩展条款及内容信息不能为空!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID == "caicyqx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "jiqshx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "jianzgchtzfbzbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "jianzgchtzfbzbx") {
                            if (null == $scope.listOptions[i].insuranceItemsOptions.data || 0 == $scope.listOptions[i].insuranceItemsOptions.data.length) {
                                ...
                                let hasGdzc = false;
                                for (let j = 0; j < $scope.listOptions[i].insuranceItemsOptions.data.length; j++) {
                                    if ($scope.listOptions[i].insuranceItemsOptions.data[j].assetscode == 'gdzc') {
                                        hasGdzc = true;
                                    }
                                }
                                if (!hasGdzc) {
                                    return layer.alert(insureTypeName + "保险标的及保险金额信息必须上传资产数据!",
                                        {skin: 'layui-layer-lan', closeBtn: 1});
                                }
                            }
                        }
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID != "huanjwrzrx" && (null == $scope.VO.insureType[i].ALL_FEE || $scope.VO.insureType[i].ALL_FEE == 0)) {
                            return layer.alert(insureTypeName + "保险金额信息不能为0!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        if (null == $scope.VO.insureType[i].FORECAST_FEE_MIN || $scope.VO.insureType[i].FORECAST_FEE_MIN == 0) {
                            return layer.alert(insureTypeName + "保费信息不能为0!",
                                {skin: 'layui-layer-lan', closeBtn: 1});
                        }
                        ...
                        if ($scope.VO.insureType[i].INSURE_TYPE_ID == "gongdzrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "diaozzrbx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongydszzrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "anqsczrx" || $scope.VO.insureType[i].INSURE_TYPE_ID == "gongysyqyjyzrbx") {
                            if (null == $scope.listOptions[i].insuranceThirdResponsibilityridOptions.data || 0 == $scope.listOptions[i].insuranceThirdResponsibilityridOptions.data.length) {
    ...
    $scope.initPageInsurance = function (i) {
        return {
            ...
            columnDefs: [
                ...
                {name: 'insureValue', displayName: '投保金额', cellFilter: 'AMOUNT_FILTER2', enableCellEdit: false,},
            ],
            data: new Array(),
            ...
            onRegisterApi: function (gridApi) {
                ...
                if (gridApi.edit) {
                    gridApi.edit.on.afterCellEdit($scope, function (rowEntity, colDef, newValue, oldValue) {
                        ...
                        if ('insurancecharge' == colDef.name) {
                            rowEntity.insurancecharge = rowEntity.insurancecharge.replaceAll(/,/g, "");
                        }
                    });
                }
            }
    ...
    $scope.initPageInsureAmount = function (i) {
        return {
            ...
            columnDefs: [
                ...
                {name: 'insuranceMoney', displayName: '第三者责任险赔偿限额（元/辆）', cellFilter: 'AMOUNT_FILTER2', enableCellEdit: true,},
            ],
            data: new Array(),
            onRegisterApi: function (gridApi) {
    ...
    $scope.initPageinsurebeHolde = function (i) {
        return {
            ...
            columnDefs: [
                ...
                {
                    name: 'BEHOLDER_INSURANCEMONEY', displayName: '保险金额', enableCellEdit: $scope.isEdit, visible: $scope.isShowBEHOLDER, cellFilter: 'AMOUNT_FILTER2'
                },
            ],
            data: new Array(),
            onRegisterApi: function (gridApi) {
                ...
                if (gridApi.edit) {
                    gridApi.edit.on.afterCellEdit($scope, function (rowEntity, colDef, newValue, oldValue) {
                        if ('BEHOLDER_INSURANCEMONEY' == colDef.name) {
                            rowEntity.BEHOLDER_INSURANCEMONEY = rowEntity.BEHOLDER_INSURANCEMONEY.replaceAll(/,/g, "");
                        }
                    });
                }
            }
    ...
    $scope.initPageGLife = function (i) {
        return {
            ...
            columnDefs: [
                ...
                {
                    name: 'insuranceMoneyEveryone', displayName: '保险金额/人(万元)', enableCellEdit: $scope.isEdit, cellFilter: 'AMOUNT_FILTER2'
                },
                {
                    name: 'insuranceMoneyAll', displayName: '总保险金额(万元)*', enableCellEdit: $scope.isEdit, cellFilter: 'AMOUNT_FILTER2'
                },
                {
                    name: 'deductible', displayName: '免赔额(元)*', enableCellEdit: $scope.isEdit, cellFilter: 'AMOUNT_FILTER2'
                },
                {
                    name: 'rate', displayName: '费率% *', enableCellEdit: $scope.isEdit
                },
                {
                    name: 'insurancePremiumEveryone', displayName: '保险费/人(元)', enableCellEdit: $scope.isEdit, cellFilter: 'AMOUNT_FILTER2'
                },
                {
                    name: 'premiumAll', displayName: '总保费(元)*', enableCellEdit: $scope.isEdit, cellFilter: 'AMOUNT_FILTER2'
                }
            ],
            data: new Array(),
            onRegisterApi: function (gridApi) {
...
app.controller('projectGridOptionsCtrl1', function ($rootScope, $scope, $http, uiGridConstants, ngDialog, ngVerify) {
    ...
    $scope.initQUERYChildren = function () {
        ...
        if ($scope.tabName == "insureCompanyGridOptions") {
            ...
            if ($scope.$parent.VO.insureType[$scope.$parent.numBer].INSURE_TYPE_ID != 'constructionPay') {
                if ($scope.ids != null) {
    ...
    $scope.initData = function () {
        $scope.customerGridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'C_0_TRADETYPE', displayName: '行业类别'},
...
app.controller('createInsuranceCtrl', function ($rootScope, $scope, $sce, $http, uiGridConstants, ngDialog, ngVerify) {
    var rows = $scope.gridApi.selection.getSelectedRows();
    $scope.insurancelist = [];
    for (var i = 0; i < rows.length; i++) {
        var issvo = rows[i];
        $scope.findOne(issvo.id);
        issvo = $scope.VO;
        var insureTypeList = issvo.insureType;
        for (let j = 0; j < insureTypeList.length; j++) {
            var vo = {};
            vo.IssueNoticeNo = issvo.IssueNoticeNo;
            vo.id = issvo.id;
            vo.name = issvo.name;
            vo.insureTypeID = insureTypeList[j].INSURE_TYPE_ID;
            $scope.insurancelist.push(vo);
        }
    }
    ;
    $scope.onCancel = function () {
    ...
    $scope.onConfirm = function () {
        $scope.createInsurance($scope.insurancelist);
    };
    $scope.removeInsurance = function (index) {
        $scope.insurancelist.splice(index, 1);
    };
    $scope.createInsuranceOne = function (index, item) {
        layer.load(2);
        $http.post($scope.basePath + "propertyInsurance/checkInsuranceno", {
            param: angular.toJson(item),
            "insureType": "unlife"
        }).success(function (response) {
            if (response && response.code == "200" && "" != response.msg) {
                ...
                var items = [];
                items.push(item);
                $http.post($scope.basePath + "issueNotice/createInsurance", {
                    items: angular.toJson(items)
                }).success(function (response) {
                    layer.closeAll('loading');
                    if (response.code == 200) {
                        layer.alert('保单生成成功!', {skin: 'layui-layer-lan', closeBtn: 1});
                        $scope.insurancelist.splice(index, 1);
                        if ($scope.insurancelist.length == 0) {
                            $scope.closeThisDialog();
                        }
                    } else {
                        layer.msg('请输入保单号!', {icon: 5});
                    }
                });
            }
        });
    };
    $scope.createInsurance = function (items) {
        layer.load(2);
        $http.post($scope.basePath + "issueNotice/createInsurance", {
            items: angular.toJson(items)
        }).success(function (response) {
    ...
    $scope.onCheckInsuranceno = function (item) {
        $http.post($scope.basePath + "propertyInsurance/checkInsuranceno", {
            param: angular.toJson(item),
            "insureType": "unlife"
        }).success(function (response) {
