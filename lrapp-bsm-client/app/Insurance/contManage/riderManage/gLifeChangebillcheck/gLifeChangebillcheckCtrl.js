 ...
 */
app.controller('gLifeChangebillcheckCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog, $location) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryAllForGrid = function (data) {
            layer.load(2);
            return $http.post($rootScope.basePath + "gLifeChangebillcheck/queryAllForGrid", {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "gLifeChangebillcheck/queryForGrid", {
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
        ...
        $scope.onDiscard = function () {
            ...
            layer.confirm('是否作废选中数据？', {
                ...
                function () {
                    layer.load(2);
                    $http.post($scope.basePath + "gLifeChangebillcheck/discard", {pk: $scope.pk, billdef: $scope.billdef}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            if (response.code == 200) {
                                //调用查询方法刷新页面
        ...
        $scope.findOne = function (pk, callback) {
            $scope.pk = pk;
            $http.post($scope.basePath + "gLifeChangebillcheck/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
        ...
        $scope.onDiscard = function () {
            layer.confirm('是否作废选中数据？', {
                ...
                function () {
                    layer.load(2);
                    $http.post($scope.basePath + "gLifeChangebillcheck/discard", {pk: $scope.pk}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            if (response.code == 200) {
                                layer.msg('作废成功!', {
        ...
        $scope.onSaveVO = function () {
            ...
            layer.load(2);
            $http.post($rootScope.basePath + "gLifeChangebillcheck/save", {
    ...
    $scope.initFunction = function () {
        ...
        $scope.nvoiceSterilisation = function () {
            if ($scope.VO.srcBillno != null && $scope.VO.correctingReasons != null && $scope.VO.correctingReasons == 1) {
                $http.post($rootScope.basePath + "gLifeChangebillcheck/nvoiceSterilisation", {data: angular.toJson($scope.VO.srcBillno)})
                    .success(function (response) {
                        if (response.code == 200) {
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.estimatepk.name', function (newVal, oldVal) {
            ...
            if (newVal && $scope.isEdit) {
                ...
                $scope.VO.estimatelinktel = $scope.VO.estimatepk.c_1_phone;
            }
        });

        //佣金变更
        // $scope.$watch('VO.feechange', function (newVal, oldVal) {
        //     if (newVal === oldVal || newVal == undefined || newVal == null) return;
        //     if (newVal && $scope.isEdit) {
        //         $http.post($scope.basePath + "changebillcheck/feechangeWatch", {data: angular.toJson($scope.VO)}).success(function (response) {
        //             $scope.putPayment(response,3,oldVal);
        //
        //         })
        //
        //     }
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "gLifeChangebillcheck/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '人身险批单信息.csv',
            columnDefs: [
                ...
                {name: 'vdef5', displayName: '保单号变更', width: 100,},
        ...
        $scope.paymentGridOptions = {
            ...
            columnDefs: [
                ...
                {
                    name: 'noPaymentMoney', displayName: '未结算金额', width: 100, enableCellEdit: false, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                },
...
//获取财务联查时url中的id
    ...
    $scope.billdef = "GLifeChangebillcheck";
