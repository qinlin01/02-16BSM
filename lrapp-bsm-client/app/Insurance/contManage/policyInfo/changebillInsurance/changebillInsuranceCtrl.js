 ...
 */
app.controller('changebillInsuranceCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog, $location) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "changebillInsurance/queryForGrid", {
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
    ...
    $scope.initFunction = function () {
        $scope.saveChangeForHistory = function () {
            $http.post($scope.basePath + "changebillInsurance/saveChangeForHistory");
        }
        ...
        $scope.getOldDetail = function (id) {
            layer.load(2);
            $http.post($scope.basePath + "changebillInsurance/findOldData", {
                pk: id
            }).success(function (response) {
                if (response && response.code == "200") {
                    $scope.card = true;
        ...
        $scope.getDetail = function (id) {
            layer.load(2);
            $http.post($scope.basePath + "changebillInsurance/findByChangeno", {
                pk: id
            }).success(function (response) {
                if (response && response.code == "200") {
                    $scope.card = true;
...
//附件批量上传
    ...
    $scope.initButton = function () {
        ...
        $scope.onCard = function (id) {
            ...
            $http.post($scope.basePath + "changebillInsurance/findOne", {
                pk: id,
                ...
                if (response && response.code == "200") {
                    $scope.card = true;
                    angular.assignData($scope.VO, response.result);
                    $scope.changeGridOptions.data = angular.copy(response.result.changebill);
                    $scope.changebill = angular.copy(response.result.changebill);
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '保单台账信息.csv',
            columnDefs: [
                {name: 'insuranceinfono', displayName: '保单信息编号', width: 100,},
                {name: 'pkProject.name', displayName: '业务/项目名称', width: 200,},
                {name: 'pkProject.code', displayName: '立项号 ', width: 100,},
                {name: 'versionNo', displayName: '批改次数 ', width: 100,},
                ...
                {name: 'operateTime', displayName: '制单时间', width: 150,},
        ...
        $scope.changeGridOptions = {
            ...
            showColumnFooter: false,
            columnDefs: [
                {name: 'vbillno', displayName: '批单信息编号', width: 150,},
                {name: 'insurancecheckno', displayName: '批单编号', width: 150,},
                {name: 'vdef1no', displayName: '批单类型', width: 100, cellFilter: 'SELECT_CHANGEBILLCHECKTYPE'},
                {name: 'changebillType', displayName: '修改内容类型', width: 100, cellFilter: 'SELECT_CHANGEBILLTYPE'},
                {name: 'correctingReasons', displayName: '批改原因', width: 100, cellFilter: 'SELECT_CORRECTINGREASONSTYPE'},
                {name: 'pkOperator.name', displayName: '经办人', width: 100,},
                {name: 'checkTime', displayName: '审核时间', width: 150,},
                {name: 'operateTime', displayName: '制单时间', width: 150,},
            ],
            data: $scope.changebill,
            onRegisterApi: function (gridApi) {
                $scope.changeGridOptions.gridApi = gridApi;
                $scope.changeGridOptions.gridApi.selection.on.rowSelectionChanged($scope, function (row, event) {
                    //行选中事件
                    var rows = $scope.changeGridOptions.gridApi.selection.getSelectedRows();
                    if (rows[0] != null && rows[0].vbillno != null) {
                        $scope.getDetail(rows[0].vbillno);
