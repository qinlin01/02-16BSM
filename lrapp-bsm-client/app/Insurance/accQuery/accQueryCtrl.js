 ...
 */
app.controller('accQueryCtrl', function ($rootScope, $scope, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            layer.load(2);
            $http.post($scope.basePath + "accQuery/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "accQuery/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
    ...
    $scope.initButton = function () {
        ...
        $scope.onView = function () {
            ...
            if ($scope.isGrid) {
                ...
                // layer.load(2);
                // $scope.findOne(rows[0].primaryKey,function (response) {
                $scope.isGrid = false;
                $scope.isBack = false;
                $scope.isEdit = false;
                $scope.isDisabled = true;
        ...
        $scope.onEdit = function () {
            ...
            if ($scope.isGrid) {
                ...
                // $scope.findOne(rows[0].pkProject);
                $scope.isBack = true;
                $scope.isEdit = true;
                $scope.isDisabled = false;
            } else {
        ...
        $scope.onAddLine = function () {
            var newData = {};
            newData["rowKey"] = Math.random() * 12345678;
            if (!$scope[$scope.selectTabName].data) {
                $scope[$scope.selectTabName].data = [];
            }
            $scope[$scope.selectTabName].data.push(newData);
        ...
        $scope.onDeleteLine = function () {
            ...
            var rowKey = delRow[0].rowKey;
            var tableData = $scope[$scope.selectTabName].data;
            var ids = [];
            for (var i = 0; i < delRow.length; i++) {
                ids.push(delRow[i].rowKey);
            }
            var arr = [];
            for (var j = 0; j < tableData.length; j++) {
                if (ids.indexOf(tableData[j].rowKey) < 0) {
                    arr.push(tableData[i]);
                }
            }
            $scope[$scope.selectTabName].data = arr;
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                {name: 'file_name', displayName: '附件名称', width: 100,},
                {name: 'code', displayName: '所属业务单据编号', width: 100,},
                {name: 'zm', displayName: '所属业务单据名称', width: 100,},
                {name: 'user_name', displayName: '上传人', width: 100,},
                {name: 'org_name', displayName: '上传单位', width: 100,},
                {name: 'operate_date', displayName: '上传日期', width: 100,},
            ],
            data: []
        };
