 ...
 */
app.controller('agencyCustomerCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, $location, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "agencyCustomer/queryForGrid", {
        ...
        $scope.findOne = function (pk,ifDesensitize) {
            $scope.pk = pk;
            $http.post($scope.basePath + "agencyCustomer/findOne", {pk: pk,ifDesensitize:ifDesensitize}).success(function (response) {
                layer.closeAll('loading');
        ...
        $scope.onSaveVO = function () {
            ...
            if ($scope.childMust) {
                layer.load(2);
                $http.post($rootScope.basePath + "agencyCustomer/save", {data: angular.toJson($scope.VO), funCode: $scope.funCode})
                    .success(function (response) {
                        if (!response.flag) {
                            $scope.isGrid = false;
                            $scope.isBack = false;
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.enumAgencyCompType', function (newVal, oldVal) {
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                $scope.VO['enumAgencyCompTypeName'] = $rootScope.SELECT.AGENCYCOMPTYPE[newVal - 1].name;
            }
        }, true);
    };
    ...
    $scope.initButton = function () {
        ...
        $scope.onSubmit = function () {
            ...
            $http.post($scope.basePath + "agencyCustomer/submit", {data: angular.toJson(rows[0])}).success(function (response) {
                if (response && response.code == 200) {
                    $scope.queryForGrid($scope.QUERY);
                    if (response) {
                        if (response.msg) {
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "agencyCustomer/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
        ...
        $scope.onSave = function () {
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    $scope.VO.enumAgencyCompTypeName = $rootScope.returnSelectName($scope.VO.enumAgencyCompType, "AGENCYCOMPTYPE");
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '保险中介公司.csv',
            columnDefs: [
                {name: 'c0Code', displayName: '中介公司编码', width: 100,},
                {name: 'c0Name', displayName: '中介公司名称', width: 100,},
                {name: 'enumAgencyCompType', displayName: '中介公司类型', width: 100, cellFilter: 'SELECT_AGENCYCOMPTYPE'},
