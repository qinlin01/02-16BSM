 ...
 */
app.controller('groupCustomerChangeCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "groupCustomerChange/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "groupCustomerChange/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
                    ...
                    if($scope.VO.update){
                        ...
                        $scope.update.push({colName: '客户级别', colOldVal: response.result.c1Level, colNewVal: 'c1Level'});
                        $scope.update.push({
                            colName: '上级管理单位',
                            colOldVal: response.result.upCustomerSw?response.result.upCustomerSw.name:'',
                            colNewVal: '',
                            colNewVals: 'upCustomerSw'
                        });
                        $scope.update.push({
                            colName: '行业类别',
                            colOldVal: response.result.tradetype && response.result.tradetype.name,
                            colNewVal: '',
                            colNewVals: 'tradetype'
                        });
                        $scope.update.push({
                            colName: '单位性质',
    ...
    $scope.initWatch = function () {
        $scope.$watch('VO.cusChangeType', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                ...
                if(newVal == 8){
                    $scope.isUse = false;
                    $scope.isUseOpp = false;
                    $scope.changeGroup = true;
                    $scope.ifGroup = false;
                    $scope.orgName = true;
                }
                if(newVal == 0){
        ...
        $scope.$watch('VO.pkCustomer.name', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                if (newVal) {
                    ...
                    $http.post($scope.basePath + "stateGridCustomer/findOne", {pk: $scope.VO.pkCustomer.pk}).success(function (response) {
                        if (response && response.code == "200") {
                            // angular.assignData($scope.VO, response.result);
                            $scope.upCustomer.pk = response.result.id;
                            $scope.VO.c0Code = response.result.c0Code;
                            $scope.VO.signcustomerno = response.result.signcustomerno;
                            $scope.VO.c0Name = response.result.c0Name;
                            $scope.VO.c1Institution = response.result.c1Institution;
                            $scope.VO.c1Level = response.result.c1Level;
                            $scope.VO.c0HasCustomerTop = response.result.c0HasCustomerTop;
                            if($scope.VO.cusChangeType==8){
                                $scope.VO.coCustomerTopName = null;
                                $scope.VO.upCustomerSw = null;
                            }else {
                                $scope.VO.coCustomerTopName = response.result.upCustomer;
                                $scope.VO.upCustomerSw = response.result.upCustomer;
                            }
                            $scope.VO.tradetype = response.result.tradetype;
                            ...
                            $scope.update.push({colName: '客户级别', colOldVal: response.result.c1Level, colNewVal: '', colNewVals: 'c1Level'});
                            $scope.update.push({
                                colName: '上级管理单位',
                                colOldVal: response.result.upCustomerSw?response.result.upCustomerSw.name:'',
                                colNewVal: '',
    ...
    $scope.initButton = function () {
        ...
        $scope.onEdit = function () {
            ...
            // $scope.update = [];
            // $scope.update.push({colName:'国网客户名称',colOldVal:$scope.VO.c0Name,colNewVal:'c0Name'});
            // $scope.update.push({colName:'注册号/统一社会信用代码',colOldVal:$scope.VO.c1Institution,colNewVal:'c1Institution'});
            // $scope.update.push({colName:'客户级别',colOldVal:$scope.VO.c1Level,colNewVal:'c1Level'});
            // $scope.update.push({colName:'上级管理单位',colOldVal:$scope.VO.upCustomerSw.name,colNewVal:'upCustomerSw'});
            // $scope.update.push({colName:'行业类别',colOldVal:$scope.VO.tradetype.name,colNewVal:'tradetype'});
            // $scope.update.push({colName:'单位性质',colOldVal:$scope.VO.enumEntkind.name,colNewVal:'enumEntkind'});
            // $scope.update.push({colName:'所在区域',colOldVal:$scope.VO.c1Province.name,colNewVal:'c1Province'});
            // $scope.update.push({colName:'地址',colOldVal:$scope.VO.c0Address,colNewVal:'c0Address'});
            // $scope.update.push({colName:'传真',colOldVal:$scope.VO.c1Fax,colNewVal:'c1Fax'});
            // $scope.update.push({colName:'公司电话',colOldVal:$scope.VO.c1Phone,colNewVal:'c1Phone'});
        };
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "groupCustomerChange/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '国网客户信息变更.csv',
            columnDefs: [
                {name: 'serial', displayName: '变更流水号', width: 100,},
                {name: 'c0Code', displayName: '国网客户编号', width: 100,},
                {name: 'signcustomerno', displayName: '国网客户代码', width: 100,},
                {name: 'c0Name', displayName: '国网客户名称', width: 100,},
                {name: 'c1Institution', displayName: '注册号/统一社会信用代码', width: 100,},
                {name: 'c1Level', displayName: '客户级别', width: 100, cellFilter: 'SELECT_CUSTOMERLEVELTYPE'},
                ...
                {name: 'cusChangeType', displayName: '变更类型', width: 100, cellFilter: 'SELECT_GWCHANGETYPE'},
    ...
    $scope.billdef = "GroupCustomerChange";
...
app.controller('customerChangeCompareGridOptionsCtrl', function ($rootScope, $scope,$sce, $http, uiGridConstants, ngDialog, ngVerify) {
});
