 ...
 */
app.controller('personalCustomerChangeCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "personalCustomerChange/queryForGrid", {
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            $http.post($scope.basePath + "personalCustomerChange/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
                    ...
                    if($scope.VO.update){
                        ...
                        $scope.update.push({
                            colName: '是否电力',
                            colOldVal: response.result.c0Ifele,
                            colNewVal: '',
                            colNewVals: 'c0Ifele'
                        });
                        $scope.update.push({
                            colName: '性别',
                            colOldVal: response.result.enumSex,
                            colNewVal: '',
                            colNewVals: 'enumSex'
                        });
                        $scope.update.push({
                            colName: '出生日期',
                            colOldVal: response.result.c1Birthday,
                            colNewVal: '',
                            colNewVals: 'c1Birthday'
                        });
                        $scope.update.push({
                            colName: '证件类型',
                            colOldVal: response.result.enumCerttype,
                            colNewVal: '',
                            colNewVals: 'enumCerttype'
                        });
                        $scope.update.push({
                            colName: '证件号码',
                            colOldVal: response.result.c1Certcode,
                            colNewVal: '',
                            colNewVals: 'c1Certcode'
                        });
                        $scope.update.push({
                            colName: '地址',
                        ...
                        $scope.update.push({
                            colName: '传真',
                            colOldVal: response.result.c1Fax,
                            colNewVal: '',
                            colNewVals: 'c1Fax'
                        });
                        $scope.update.push({
        ...
        $scope.onSaveVO = function () {
            layer.load(2);
            $http.post($rootScope.basePath + "personalCustomerChange/save", {data: angular.toJson($scope.VO), funCode:$scope.funCode})
                .success(function (response) {
                    if (!response.flag) {
                        $scope.isGrid = false;
                        $scope.isBack = false;
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.pkCustomer.name', function (newVal, oldVal) {
            ...
            if ($scope.isEdit) {
                if (newVal) {
                    $http.post($scope.basePath + "personalCustomer/findOne", {pk: $scope.VO.pkCustomer.pk}).success(function (response) {
                        if (response && response.code == "200") {
                            $scope.VO.c0Code = response.result.c0Code;
                            $scope.VO.c0Name = response.result.c0Name;
                            $scope.VO.c0Ifele = response.result.c0Ifele;
                            $scope.VO.enumSex = response.result.enumSex;
                            $scope.VO.c1Birthday = response.result.c1Birthday;
                            $scope.VO.enumCerttype = response.result.enumCerttype;
                            $scope.VO.c1Certcode = response.result.c1Certcode;
    ...
    $scope.initButton = function () {
     ...
     /*   $scope.onQuery = function () {
        ...
        $scope.onEdit = function () {
            ...
            // $scope.update = [];
            // $scope.update.push({colName:'客户名称',colOldVal:$scope.VO.c0Name,colNewVal:'',colNewVal:'c0Name'});
            // $scope.update.push({colName:'是否电力',colOldVal:$scope.VO.c0Ifele,colNewVal:'c0Ifele'});
            // $scope.update.push({colName:'性别',colOldVal:$scope.VO.enumSex,colNewVal:'enumSex'});
            // $scope.update.push({colName:'出生日期',colOldVal:$scope.VO.c1Birthday,colNewVal:'c1Birthday'});
            // $scope.update.push({colName:'证件类型',colOldVal:$scope.VO.enumCerttype,colNewVal:'enumCerttype'});
            // $scope.update.push({colName:'证件号码',colOldVal:$scope.VO.c1Certcode,colNewVal:'c1Certcode'});
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "personalCustomerChange/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            exporterCsvFilename: '个人客户信息变更.csv',
            columnDefs: [
                {name: 'serial', displayName: '变更流水号', width: 100,},
                {name: 'c0Code', displayName: '个人客户编号', width: 100,},
    ...
    $scope.billdef = "PersonalCustomerChange";
