...
app.controller('againChangebillInsuranceCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog, $location) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "againChangebillInsurance/queryForGrid", {
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
    ...
    $scope.initFunction = function () {
        $scope.getSelectDetail = function () {
            ...
            if (rows[0].vbillno != null) {
                $scope.getDetail(rows[0].vbillno);
            }
        }
        $scope.getOldDetail = function (id) {
            layer.load(2);
            $http.post($scope.basePath + "againChangebillInsurance/findOldData", {
                pk: id
            }).success(function (response) {
                if (response && response.code == "200") {
                    $scope.card = true;
        ...
        $scope.getDetail = function (id) {
            layer.load(2);
            $http.post($scope.basePath + "againChangebillInsurance/findByChangeno", {
                pk: id
            }).success(function (response) {
                if (response && response.code == "200") {
                    $scope.card = true;
...
//附件批量上传
    ...
    $scope.initButton = function () {
        ...
        $scope.onCard = function (id) {
            ...
            $http.post($scope.basePath + "againChangebillInsurance/findOne", {
                pk: id,
