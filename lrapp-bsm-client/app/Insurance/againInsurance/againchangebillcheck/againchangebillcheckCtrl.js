 ...
 */
app.controller( 'againchangebillcheckCtrl', function ($rootScope, $scope, $sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog, $location) {
    ...
    $scope.initHttp = function () {
        ...
        $scope.queryAllForGrid = function (data) {
            layer.load(2);
            return $http.post($rootScope.basePath + "againchangebillcheck/queryAllForGrid", {
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post($scope.basePath + "againchangebillcheck/queryForGrid", {
                params: angular.toJson(data),
                page: $scope.gridApi ? $scope.gridApi.page : $scope.gridOptions.page,
                pageSize: $scope.gridApi ? $scope.gridApi.pageSize : $scope.gridOptions.pageSize,
                fields: angular.toJson($scope.queryData)
        ...
        $scope.onDiscard = function () {
            ...
            layer.confirm('是否作废选中数据？', {
                ...
                function () {
                    layer.load(2);
                    $http.post($scope.basePath + "againchangebillcheck/discard", {pk: $scope.pk, billdef: $scope.billdef}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            if (response.code == 200) {
                                //调用查询方法刷新页面
        ...
        $scope.findOne = function (pk) {
            $scope.pk = pk;
            layer.load(2);
            $http.post($scope.basePath + "againchangebillcheck/findOne", {pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200") {
        ...
        $scope.onSaveVO = function () {
            ...
            layer.load(2);
            $http.post($rootScope.basePath + "againchangebillcheck/save", {data: angular.toJson($scope.VO), funCode: $scope.funCode})
                .success(function (response) {
                    ...
                    if (!response.flag) {
                        ...
                        $scope.VO.vbillno = response.result.vbillno;
                        $scope.VO.insurancecheckno = response.result.insurancecheckno;
    ...
    $scope.initFunction = function () {
        ...
        $scope.nvoiceSterilisation = function () {
            if ($scope.VO.srcBillno != null && $scope.VO.correctingReasons != null && $scope.VO.correctingReasons == 1) {
                $http.post($rootScope.basePath + "againchangebillcheck/nvoiceSterilisation", {data: angular.toJson($scope.VO.srcBillno)})
                    .success(function (response) {
                        if (response.code == 200) {
        ...
        $scope.putPayment = function (response, typeNum) {
            ...
            if (response) {
                ...
                /* if(response.payment.length == 0){
                     ...
                     var length = response.payment.length;
                     var i=0;
                     var flags=0;
                     for (var j = 0; j < length; j++) {
                         typeMoneyNO = response.payment[j].typeMoneyNO;
                         if(typeMoneyNO == typeNum){
                             let paymentDatas  = angular.fromJson(response.payment[j]);
                             $scope.paymentGridOptions.data.push(paymentDatas);
                             i++;
                             flags=1;
                         }
                     }
    ...
    $scope.initWatch = function () {
        ...
        $scope.$watch('VO.srcBillno.insuranceinfono', function (newVal, oldValue) {
            ...
            if ($scope.isEdit) {
                layer.load(2);
                $http.post($scope.basePath + "againchangebillcheck/srcBillnoWatch", {data: angular.toJson($scope.VO)}).success(function (response) {
                    if (response) {
        ...
        //佣金变更
        $scope.watchfeechange = function (event) {
            if (!$scope.isEdit) {
                return;
            }
            if ($scope.isEdit) {
                ...
                if (textValue == 0) {
                    ...
                    for (var i = 0; i < array.length; i++) {
                        for (var j = 0; j < array.length; j++) {
                            if (array[j].typeMoneyNO == array[i].typeMoneyNO && array[i].typeMoneyNO == 3 && array[j].typeMoneyNO == 3) {
                                array.splice(i, 1);
                                i = -1;
                                break;
                            }
                ...
                layer.load(2);
                $http.post($scope.basePath + "againchangebillcheck/feechangeWatch", {data: angular.toJson($scope.VO)}).success(function (response) {
        ...
        //应收保费变更
        $scope.watchvdef2change = function (event) {
            if (!$scope.isEdit) {
                return;
            }
            if ($scope.isEdit) {
                ...
                if (textValue == 0) {
                    ...
                    for (var i = 0; i < array.length; i++) {
                        for (var j = 0; j < array.length; j++) {
                            if (array[j].typeMoneyNO == array[i].typeMoneyNO && array[i].typeMoneyNO == 1 && array[j].typeMoneyNO == 1) {
                                array.splice(i, 1);
                                i = -1;
                                break;
                            }
                ...
                layer.load(2);
                $http.post($scope.basePath + "againchangebillcheck/vdef2Watch", {data: angular.toJson($scope.VO)}).success(function (response) {
        ...
        //应解付保费变更
        $scope.watchvvdef3change = function (event) {
            if (!$scope.isEdit) {
                return;
            }
            if ($scope.isEdit) {
                ...
                if (textValue == 0) {
                    ...
                    for (var i = 0; i < array.length; i++) {
                        for (var j = 0; j < array.length; j++) {
                            if (array[j].typeMoneyNO == array[i].typeMoneyNO && array[i].typeMoneyNO == 2 && array[j].typeMoneyNO == 2) {
                                array.splice(i, 1);
                                i = -1;
                                break;
                            }
                ...
                layer.load(2);
                $http.post($scope.basePath + "againchangebillcheck/vdef3Watch", {data: angular.toJson($scope.VO)}).success(function (response) {
        ...
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post($scope.basePath + "againchangebillcheck/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
                        if (response && response.code == "200") {
                            $scope.queryForGrid($scope.QUERY);
                            layer.msg('删除成功!', {
        ...
        * */
        ...
        $scope.onDeleteLinepaymentGridOptions = function () {
            ...
            if ($scope.VO.srcBillno != null && $scope.VO.correctingReasons != null && $scope.VO.correctingReasons == 1) {
                for (var i = 0; i < $scope.paymentGridOptions.data.length; i++) {
                    for (var j = 0; j < delRow.length; j++) {
                        if (delRow[j].stages == $scope.paymentGridOptions.data[i].stages) {
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'srcBillno.insuranceinfono', displayName: '保单编号', width: 100,},
                ...
                {name: 'correctingReasons', displayName: '批改原因', width: 100, cellFilter: 'SELECT_CORRECTINGREASONSTYPE'},
        ...
        $scope.paymentGridOptions = {
            ...
            columnDefs: [
                ...
                {
                    name: 'planMoneyOld', displayName: '计划金额', width: 100, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                },
                {
                    name: 'factMoneyOld', displayName: '已结算金额', width: 100, enableCellEdit: false, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                },
                {
                    name: 'noPaymentMoneyOld', displayName: '未结算金额', width: 100, enableCellEdit: false, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                },
                {
                    name: 'planMoney', displayName: '计划金额（人民币）', width: 100, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    } , enableCellEdit: false
                },
                {
                    name: 'factMoney', displayName: '已结算金额（人民币）', width: 100, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                    , enableCellEdit: false
                ...
                {
                    name: 'noPaymentMoney', displayName: '未结算金额（人民币）', width: 100, cellFilter: 'AMOUNT_FILTER',
                    cellClass: function () {
                        return "lr-text-right"
                    }
                    , enableCellEdit: false
...
//获取财务联查时url中的id
    ...
    $scope.billdef = "AgainChangebillcheck";
