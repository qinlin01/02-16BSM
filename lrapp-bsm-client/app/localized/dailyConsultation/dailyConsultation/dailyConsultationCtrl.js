app.controller('dailyConsultationCtrl', function ($rootScope, $scope,$sce, $http, $stateParams, uiGridConstants, ngDialog, ngVerify, activitiModal, workFlowDialog, $location) {
    $scope.initData =function (data) {
        ...
        $scope.entityVO = 'nc.vo.busi.dailyConsultationVO';
        //主表数据
    ...
    $scope.initHttp=function(){
        ...
        $scope.queryForGrid = function (data) {
            ...
            layer.load(2);
            $http.post(localizedPath  + "dailyConsultationController/queryForGrid", {
    ...
    $scope.initButton = function(){
        ...
        * */
        $scope.onDelete = function () {
            ...
            layer.confirm('是否删除选中数据？', {
                ...
                function () {
                    ...
                    layer.load(2);
                    $http.post(localizedPath  + "dailyConsultationController/delete", {ids: angular.toJson(ids)}).success(function (response) {
                        layer.closeAll('loading');
        ...
        $scope.findOne = function (pk,callback) {
            $scope.pk = pk;
            $http.post(localizedPath  + "dailyConsultationController/findOne",{pk: pk}).success(function (response) {
                layer.closeAll('loading');
                if (response && response.code == "200"){
        ...
        * */
        ...
        $scope.onCard = function (id) {
            ...
            $http.post( localizedPath  + "dailyConsultationController/findOne",{
                pk: id,
            ...
            ngVerify.check('appForm', function (errEls) {
                if (errEls && errEls.length) {
                    ...
                    if (data) {
                        ...
                        $http.post( localizedPath  + "dailyConsultationController/save",{data:angular.toJson(data),tablename:tablename}).success(function(response) {
                            if(response.code == 200){
                                $scope.isGrid = false;
                                $scope.isBack = false;
                                $scope.isEdit = false;
        ...
        $scope.onTemporary = function (data) {
            $scope.VO.dealAttachmentB = $scope.dealAttachmentBGridOptions.data;
                    if (data) {
                        data=$scope.VO;
                        var tablename = $scope.table_name;
                        $http.post( localizedPath  + "dailyConsultationController/onTemporary",{data:angular.toJson(data),tablename:tablename}).success(function(response) {
                            if(response.code == 200){
                                $scope.isGrid = false;
                                $scope.isBack = false;
                                $scope.isEdit = false;
    ...
    $scope.initPage = function () {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {name: 'consultTime', displayName: '咨询日期'},
                ...
                {name: 'consultationReply', displayName: '咨询内容'},
    ...
    $scope.initWatch = function(){
        $scope.$watch('VO.consultTime', function (newVal, oldVal) {
            if (newVal === oldVal || newVal == undefined || newVal == null) return;
            if ($scope.isEdit) {
                if($scope.VO.consultTime){
                    $scope.VO.year = $scope.VO.consultTime.split("-")[0];
                }
            }
        }, true);
    }

    $scope.table_name = " lr_service_record";
    $scope.billdef = "DailyConsultation";
